...
CLIENT_MONITOR_AUTONOMOUS_PLAN_INSTRUCTIONS = (
    ...
    "for live prices (`candidates[]`, `price` samples), indices, positions, and headlines.\n"
    "- Think deeply: use CandidateDebate (3–6 sentences) comparing ALL symbols — momentum, levels, "
    "news catalysts, relative strength, and risk/reward before any recommendation.\n"
    "- Cross-check headlines with websearch/webfetch and Finnhub MCP when thin or stale.\n"
    "- Emit TradeDecision with `confidence_pct` (0–100) for your top setup even if you do not enter.\n"
    "- Emit InsightCards (highlights/lowlights/cautions) grounded in the batch + research.\n"
    "- Use ButtonRow only in Plan mode for user choices (enter / wait / threshold). Do NOT place orders.\n"
    "No markdown prose."
)
...
def _execute_monitor_instructions() -> str:
    ...
    return (
        ...
        "1) CandidateDebate — 4–8 sentences: compare EVERY symbol in `candidates[]` using live candles/samples, "
        "index drift, positions, and headlines. Rank setups; name the single best risk/reward.\n"
        "2) Websearch + Finnhub MCP for catalysts, earnings, sector news — fold into debate + InsightCards.\n"
        "3) TradeDecision — one line action + REQUIRED `confidence_pct` (0–100) for your primary call.\n"
        "4) InsightCards — highlights/lowlights/cautions (include sources in cautions when from web).\n\n"
        "ENTRY RULES (no open position on symbol):\n"
        f"- If best setup confidence_pct >= {threshold}: emit fenced JSON ai_action "
        "autonomous_entry (NOT ButtonRow, NOT StrategySetupForm). Include full deploy payload:\n"
        ...
        "- Use thread broker/account_env from focus/metadata.\n\n"
        "EXIT RULES (open position):\n"
        "- Thesis breaks, stop deteriorating, or target met → TradeDecision + ai_action exit_strategy "
        "(execution_id, symbol, reason) → trade_complete when flat.\n\n"
        "NEVER emit ButtonRow or StrategySetupForm in execute-mode monitor. No markdown prose."
    )
