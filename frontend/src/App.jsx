...
function Toast({ order }) {
  ...
  const icon = isBuy ? '▲' : '▼'
  const pnlBit = !isBuy ? ` P&L: ₹${order.pnl}` : ''
  return (
    <div className={`toast-enter px-4 py-2.5 rounded-lg text-xs font-semibold shadow-lg flex items-center gap-2 text-white ${isBuy ? 'bg-green/90' : 'bg-red/90'}`}>
      ...
      {order.type} {order.qty} @ ₹{order.price.toFixed(2)}{pnlBit}
    </div>
  )
}
...
export default function App() {
  ...
  const renderTick = useCallback((candle, order, serverStats, levels) => {
    ...
    if (order) {
      markersRef.current.push({
        ...
        text: `${order.type} ₹${order.price.toFixed(2)}`,
      })
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {/* Header */}
      <header className="bg-secondary border-b border-border px-6 py-3 flex items-center justify-between shrink-0">
        <h1 className="text-base font-semibold">
          <span className="text-accent">◆</span> Backtest Strategy Simulator
        </h1>
              ...
              {[...portfolio].sort((a, b) => a.tradingsymbol.localeCompare(b.tradingsymbol)).map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { setSelected(item); setView('backtest') }}
          ...
          {!selected ? (
                  ...
                  <tbody>
                    {getSortedPortfolio().map((item) => (
                      <tr key={item.symboltoken} className="border-b border-border/30 hover:bg-accent/5 transition-colors">
                        ...
                        <td className="py-2 px-3 text-right">₹{item.ltp}</td>
            ...
            <>
              ...
              <div className="grid grid-cols-7 gap-2 p-3 bg-secondary border-b border-border shrink-0">
                <StatCard label="Net P&L" value={`₹${stats.netPnl.toFixed(0)}`} colorClass={pnlColor(stats.netPnl)} />
                ...
                <StatCard label="Total Fees" value={`₹${stats.totalFees.toFixed(0)}`} colorClass="text-red" />
                <StatCard label="Shares Held" value={stats.shares} />
                <StatCard label="Funds" value={`₹${stats.funds.toFixed(0)}`} />
              </div>

              {/* Back / Re-configure buttons */}
              <div className="flex items-center gap-3 px-3 py-1.5 bg-secondary border-b border-border shrink-0">
                <button onClick={() => { stopStream(); setBacktestDone(false); setSelected(null); setView('portfolio') }}
                  className="text-[10px] text-text-secondary hover:text-text-primary transition-colors">
                  ← Portfolio
                </button>
                      ...
                      return displayedOrders.map((o, i) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-border/20 hover:bg-accent/5">
                            ...
                            <td className="py-1 px-3 text-right">₹{o.price.toFixed(2)}</td>
                            <td className="py-1 px-3 text-right">{o.qty}</td>
                            <td className="py-1 px-3 text-right">₹{o.amount.toFixed(0)}</td>
                            <td className="py-1 px-3 text-right text-red">
                              {o.type === 'SELL' && o.fee ? `₹${o.fee}` : ''}
                            </td>
                            <td className={`py-1 px-3 text-right ${o.type === 'SELL' ? pnlColor(o.pnl) : ''}`}>
                              {o.type === 'SELL' ? `₹${o.pnl}` : ''}
                            </td>
                            <td className={`py-1 px-3 text-right ${o.type === 'SELL' ? pnlColor(net) : ''}`}>
                              {o.type === 'SELL' ? `₹${net.toFixed(0)}` : ''}
                            </td>
