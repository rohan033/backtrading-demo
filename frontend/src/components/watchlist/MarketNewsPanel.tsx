...
export default function MarketNewsPanel({ className = '' }: Props) {
  ...
  return (
    <section
      className={`flex min-h-0 flex-col overflow-hidden bg-secondary ${className}`}
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-border/70 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          <Globe2 className="h-3.5 w-3.5 shrink-0 text-accent" aria-hidden="true" />
          ...
          <select
            ...
            className="h-7 max-w-[6.5rem] cursor-pointer rounded-md border border-border bg-card px-1.5 text-[10px] font-semibold text-text-primary outline-none focus:border-accent/40"
          >
      ...
      <div className="shrink-0 border-b border-border/50 px-4 py-1.5">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        {loading && items.length === 0 ? (
          <p className="px-1 py-3 text-[11px] text-text-secondary">Loading market news…</p>
        ) : error && items.length === 0 ? (
          <p className="px-1 py-3 text-[11px] text-red">{error}</p>
        ) : items.length === 0 ? (
          <p className="px-1 py-3 text-[11px] text-text-secondary">No market news found.</p>
        ) : (
          <>
            {error ? (
                ...
                <li key={item.id}>
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-text-secondary">
                      {item.category ? (
                        <span className="rounded bg-muted/30 px-1 py-0.5 text-[9px] uppercase tracking-wide">
                          {item.category}
                        </span>
                      ) : null}
